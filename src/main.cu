 ...
 * 	6- Evaluate forces between each pair of particles (derivative of L-J potential -> based on distance and pair of coefficients):
 * 		Calculating the derivative equation
 *	 OR 	Estimate forces using table of derivatives samples(closest distance entry)
...
bool analytic = false;    //SOLVE EQUATIONS EACH STEP  
bool results = false;  
...
void derivative_E_r_analytic_cpu(double* dEr, double* r, double cut, int* item_to_type, int num_samples_r,
...
__global__ 
void derivative_E_r_analytic(double* dEr, double* r, double cut, int* item_to_type, int num_samples_r,
...
__global__ 
void E_r_analytic(double* Er, double* r, double cut, int* item_to_type, int num_samples_r,
	       double* EPS, double* SIG, int width, int height)
{
      /* Elemento de la matriz a calcular */
      unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;	/** particula 2 **/
...
//****************************** 
  ...
  /******************************/
  /**Variables y memoria**/
    int num_samples_r = TEXTURE_MEM_SIZE/(sizeof(float));	// cant of original sample values (máximo permitido por mem de textura)
    ...
    //else
      cudaMalloc(&d_LJPot, num_samples_r_size * num_types);
    
    
    
    /** Rellenamos datos con CUDA **/
    //CANTIDAD TOTAL DE THREADS:  EN X=num_samples_r     EN Y=num_types 
    ...
    if(analytic){
		if(CPU){   //VERSION ANALITICA SOBRE CPU
			...
			for (x=0;x<cant_particles;x++)
				...
				derivative_E_r_analytic_cpu(h_dEr, h_distance_r, cut, h_item_particle, num_samples_r, h_epsilon, h_sigma, width, height,x,y); 
		...
		if( err != cudaSuccess)
			...
			out<<"termino todo"<< endl;
				...
				derivative_E_r_analytic<<<dimGrid, dimBlock>>>(d_dEr, d_distance_r, cut, d_item_particle, num_samples_r, d_EPS, d_SIG, width, height);
	       //if(step %100 ==0)
	       E_r_analytic<<<dimGrid, dimBlock>>>(d_Er, d_distance_r, cut, d_item_particle, num_samples_r, d_EPS, d_SIG, width, height);
           ...
           if(derivative){
			 ...
			 double *h_dEr=(double *) malloc(s_size*cant_particles);
		        ...
		        for (x=0;x<cant_particles;x++)
		          for(y=0;y<cant_particles;y++)
		            direct_derivative_E_r_cpu(h_dLJPot,h_dEr, h_distance_r,cut,h_item_particle, num_samples_r,num_types,width,height, x, y );
		       ...
		       //mando los resultados a gpu   
			    ...
			    if(text)
			        direct_derivative_E_r<<<dimGrid, dimBlock>>>(d_dEr, d_distance_r, cut, d_item_particle, num_samples_r, num_types, width, height);
			   else{
			direct_derivative_E_r_memory<<<dimGrid, dimBlock>>>(d_dLJPot, d_dEr, d_distance_r, cut, d_item_particle, num_samples_r, num_types, width, height);				
			//out<<"Salio"<<endl;  
		          	}
		 // if(step % 100 == 0)
                E_r_analytic<<<dimGrid, dimBlock>>>(d_Er, d_distance_r, cut, d_item_particle, num_samples_r, d_EPS, d_SIG, width, height); 
			 ...
			 double *h_dEr=(double *) malloc(s_size*cant_particles);
                          ...
                          for (x=0;x<cant_particles;x++)
                            for(y=0;y<cant_particles;y++)
			      derivative_E_r_cpu(h_LJPot,h_dEr, h_distance_r, cut, h_item_particle, num_samples_r, num_types,width, height,x,y);	
		        //mando los resultados a gpu   
                        cudaMemcpy( d_dEr,h_dEr, s_size, cudaMemcpyHostToDevice);
			free(h_dEr);
				...
				//out<<"Entro a text - tabla "<<endl;
			        derivative_E_r<<<dimGrid, dimBlock>>>(d_dEr, d_distance_r, cut, d_item_particle, num_samples_r, num_types, width, height);
			      //out<<"salio text tabla"<<endl;
			     }
			     else     //USO DIRECTAMENTE EL ARREGLO EN MEMORIA DE GPU
 			      derivative_E_r_memory<<<dimGrid, dimBlock>>>(d_LJPot, d_dEr, d_distance_r, cut, d_item_particle, num_samples_r, num_types, width, height);
			}	
               //if(step %100 == 0)
		 E_r_analytic<<<dimGrid, dimBlock>>>(d_Er, d_distance_r, cut, d_item_particle, num_samples_r, d_EPS, d_SIG, width, height);
