...
#define TEXTURE_MEM_SIZE	50000
#define DIF_FINITAS_DELTA	4

/** Variables físicas **/
#define CANT_TYPES		70
#define MAx			90		
#define MIn			0.01
#define DIST			(MAx - MIn)
...
__global__ void derivative_E_r_memory(float* LJPot,double* dEr, double* r, double cut, int* item_to_type,
                    int cant_samples_r, int cant_types, int width, int height )
{
	...
	result=0;
      ...
      /** type of particles **/
      int t_o_p_1 =  item_to_type[y] * cant_types;     //this one decides which subMatrix to use
      int t_o_p_2 =  item_to_type[x] + t_o_p_1;  //this one decides which row on these
      int posInicial = t_o_p_2 * cant_samples_r;
     ...
     // float index_x = (float)((double) (r[y*width+x] - MIn) * (double) cant_samples_r / DIST + 0.5);    // convert  r  to   x
      ...
      // int index=0;
      double superior=erre + (DIF_FINITAS_DELTA*DIST/cant_samples_r);
      double inferior=erre - (DIF_FINITAS_DELTA*DIST/cant_samples_r);
      int indexsup=posInicial + ((superior-MIn)*(cant_samples_r/DIST));
      int indexinf=posInicial + ((inferior-MIn)*(cant_samples_r/DIST));

      if(superior > MAx)
        indexsup=posInicial + cant_samples_r - 1;
      if(superior<MIn)
        indexsup=posInicial;
      if(inferior<MIn)
        indexinf=posInicial;
      if(inferior>MAx)
        indexinf=posInicial + cant_samples_r - 1;
...
void derivative_E_r_cpu(float* LJPot,double* dEr, double* r, double cut, int* item_to_type,
		    ...
		    int cant_samples_r, int cant_types, int width, int height)
{
	...
	result=0;
      ...
      /** r = (MAX-MIN) * X / N  + MIN    **/
      /** x = (r-MIN) * N / (MAX-MIN)     **/
      float index_x = (float)((double) (erre - MIn) * (double) cant_samples_r / DIST + 0.5);	// convert  r  to   x
      /*
...
__global__ void direct_derivative_E_r_memory(float* dLJPot,double* dEr, double* r, double cut,int* item_to_type,
                    int cant_samples_r, int cant_types, int width, int height)
{
      /* Elemento de la matriz a calcular */
      unsigned int x = blockIdx.x * blockDim.x + threadIdx.x; /** particula 2 **/
	...
	result=0;
      ...
      int t_o_p_1 = item_to_type[y] * cant_types;       //this one decides which subMatrix to use
      int t_o_p_2 =  item_to_type[x] + t_o_p_1; //this one decides which row on these
      int posInicial=t_o_p_2 * cant_samples_r;   //comienzo de la fila??
      /** Convierto r a subíndice de matriz de lennard-jones **/
      /** r = (MAX-MIN) * X / N  + MIN    **/
      /** x = (r-MIN) * N / (MAX-MIN)     **/
      //float index_x = (float)((double) (r[y*width+x] - MIn) * (double) cant_samples_r / DIST + 0.5);  // convert  r  to   x
      //int index=0;
      int posMax=cant_samples_r -2;
      
      float sesgo=(erre-MIn) *(cant_samples_r/DIST);
	if(sesgo>posMax)
...
/*
      if(erre > MAx)
        ...
        if(erre<MIn)
          ...
          int sesgoSup=ceil((erre-MIn)*(cant_samples_r/DIST));
          int sesgoInf= floor((erre-MIn)*(cant_samples_r/DIST));
          float y1=dLJPot[posInicial + sesgoSup];
          float y0=dLJPot[posInicial + sesgoInf];
	  double x0=sesgoInf*cant_samples_r /DIST;
	  double a = (y1 - y0) / (1);
  	  double b = -a*x0 + y0;
   	  double ybuscado = a *((erre-MIn)*(cant_samples_r/DIST))  + b;
...
/*********************************************
 * ESTA FUNCION RECIBE CALCULA dER A PARTIR DE LOS DATOS TABULADOS DE LA MATRIZ EN MEMORIA DE CPU (dHLJPot)
 * *****************************************/


 
void direct_derivative_E_r_cpu(float* dLJPot,double* dEr, double* r, double cut,int* item_to_type,
		    int cant_samples_r, int cant_types, int width, int height, int x, int y)
{
      /* Elemento de la matriz a calcular */
      //unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;	/** particula 2 **/
	...
	if(erre<MIn)
	  ...
	  int sesgo=(erre-MIn)*(cant_samples_r/DIST);
...
void derivative_E_r_analytic_cpu(double* dEr, double* r, double cut, int* item_to_type, int cant_samples_r,
...
if(!CPU){
  if(!text){
	  if(derivative){
		err= cudaMalloc(&d_dLJPot, cant_samples_r_size * cant_types*cant_types); 
		if( err != cudaSuccess)
			{
     			printf("CUDA error paso1: %s\n", cudaGetErrorString(err));
			}	    
		err=cudaMemcpy( d_dLJPot, h_dLJPot, cant_types * cant_types * cant_samples_r_size, cudaMemcpyHostToDevice);
		if( err != cudaSuccess)
			{
		...
		err=	 cudaMalloc(&d_LJPot, cant_samples_r_size * cant_types*cant_types);
		if( err != cudaSuccess)
		{
		     printf("CUDA error paso3: %s\n", cudaGetErrorString(err));
		}	
		err=	 cudaMemcpy( d_LJPot, h_LJPot, cant_types * cant_types * cant_samples_r_size, cudaMemcpyHostToDevice);
		if( err != cudaSuccess)
		{
    ...
    if(analytic){
		if(CPU){   //VERSION ANALITICA SOBRE CPU
			...
			for (x=0;x<cant_particles;x++)
				...
				derivative_E_r_analytic_cpu(h_dEr, h_distance_r, cut, h_item_particle, cant_samples_r, h_epsilon, h_sigma, width, height,x,y); 
		...
		if( err != cudaSuccess)
           ...
           if(derivative){
			 ...
			 double *h_dEr=(double *) malloc(s_size*cant_particles);
		        ...
		        for (x=0;x<cant_particles;x++)
		          for(y=0;y<cant_particles;y++)
		            direct_derivative_E_r_cpu(h_dLJPot,h_dEr, h_distance_r,cut,h_item_particle, cant_samples_r,cant_types,width,height, x, y );
			...
			direct_derivative_E_r_memory<<<dimGrid, dimBlock>>>(d_dLJPot, d_dEr, d_distance_r, cut, d_item_particle, cant_samples_r, cant_types, width, height);				
			//out<<"Salio"<<endl;  
		          	}
		 // if(step % 100 == 0)
                E_r_analytic<<<dimGrid, dimBlock>>>(d_Er, d_distance_r, cut, d_item_particle, cant_samples_r, d_EPS, d_SIG, width, height); 

		} else {    //USO TABLAS DE POTENCIALES Y DIFERENCIAS FINITAS
			 ...
			 double *h_dEr=(double *) malloc(s_size*cant_particles);
                          ...
                          for (x=0;x<cant_particles;x++)
                            for(y=0;y<cant_particles;y++)
			      derivative_E_r_cpu(h_LJPot,h_dEr, h_distance_r, cut, h_item_particle, cant_samples_r, cant_types,width, height,x,y);	
		        //mando los resultados a gpu   
                        cudaMemcpy( d_dEr,h_dEr, s_size, cudaMemcpyHostToDevice);
			free(h_dEr);
				...
				//out<<"Entro a text - tabla "<<endl;
			      ...
			      //out<<"salio text tabla"<<endl;
 			      ...
 			      derivative_E_r_memory<<<dimGrid, dimBlock>>>(d_LJPot, d_dEr, d_distance_r, cut, d_item_particle, cant_samples_r, cant_types, width, height);
			}	
               //if(step %100 == 0)
		 E_r_analytic<<<dimGrid, dimBlock>>>(d_Er, d_distance_r, cut, d_item_particle, cant_samples_r, d_EPS, d_SIG, width, height);
...
//		    if(step % 10000 == 0)
  ...
  if(text)
	...
	taim << cut << " " << (double) (tv2.tv_usec - tv1.tv_usec) / 1000000 + (double) (tv2.tv_sec - tv1.tv_sec)<< " " << titulo << endl;
    }
